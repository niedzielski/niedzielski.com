...
export function parseArticle(
  ...
  const matches = yamlMarkdown.match(/(?:\s*^---$(?<yaml>.*)^---$)?(?<md>.*)/ms)
...
const metaKeys: ReadonlySet<string> = new Set(<(keyof Meta)[]>[
  'dateModified',
  'datePublished',
  'keywords',
  'headline',
...
function parseMeta(url: string, val: JSONValue | undefined): Meta {
  ...
  if (unknownKeys.length > 0)
    throw Error(
      `"${url}" metadata contains unknown properties: ${unknownKeys.join(
        ', '
      )}.`
    )
